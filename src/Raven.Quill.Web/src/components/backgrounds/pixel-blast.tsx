...
/** Null where the browser refuses WebGL, e.g. without a usable GPU: the field is decoration, so it is skipped. */
function tryCreateRenderer(antialias: boolean) {
    try {
...
export default function PixelBlast({
    ...
    useEffect(() => {
        ...
        const renderer = tryCreateRenderer(antialias);
        if (!renderer) {
            return;
        }
